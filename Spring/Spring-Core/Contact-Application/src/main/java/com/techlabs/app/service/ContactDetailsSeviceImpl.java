...
public class ContactDetailsSeviceImpl implements ContactDetailsService {
    ...
    @Override
    public PagedResponse<ContactDetailsDTO> getAllContactDetailsOfContact(Long contactId, int page, int size, String sortBy, String direction) {
        Sort sort = direction.equalsIgnoreCase(Sort.Direction.DESC.name()) ?
                Sort.by(sortBy).descending() : Sort.by(sortBy).ascending();
        Pageable pageable = PageRequest.of(page, size, sort);

        Contact contact = contactRepository.findContactByIdAndIsActive(contactId, true).orElseThrow(() -> {
            logger.error("Contact with ID: {} is not active", contactId);
            return new ContactRelatedException("Contact with ID " + contactId + " is not active");
        });
        ...
        if (!user.getContacts().contains(contact)) {
            logger.error("There is no contact with ID : {} in contact list of user with ID : {}", contactId, user.getId());
            throw new ContactRelatedException("There is no contact with ID : " + contactId
                    + " in contact list of user with ID : " + user.getId());
        }

        Page<ContactDetails> contactDetailsPage = contactDetailsRepository.findByContact(contact, pageable);
        List<ContactDetails> contactDetailsList = contactDetailsPage.getContent();
        List<ContactDetailsDTO> responseDTOList = mapper.getContactDetailsDTOList(contactDetailsList);

        return new PagedResponse<>(responseDTOList, contactDetailsPage.getNumber(), contactDetailsPage.getSize(),
                contactDetailsPage.getTotalElements(), contactDetailsPage.getTotalPages(), contactDetailsPage.isLast());
    }
